...
const HeaderSwapButton: React.FC<Props> = ({ isSuccessPrice, isFetchingPrice, refetchPrice }) => {
	...
	useEffect(() => {
		if (arrowRef.current && isSuccessPrice && !isFetchingPrice) {
			arrowRef.current?.beginElement()
			arrowRef.current?.setAttribute("repeatCount", "indefinite")
		}
	}, [arrowRef, isSuccessPrice, isFetchingPrice])
		...
		<>
			<div className="w-full flex items-center justify-between bg-transparent mb-2">
				...
				<div className="header-icon-wrapper">
					<div className="header-icon" onClick={refetchPrice} style={{ pointerEvents: isSuccessPrice ? "auto" : "none" }}>
						<svg
							vtu-c443=""
							id="arrow_loading"
